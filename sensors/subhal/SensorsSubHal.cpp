...
constexpr int32_t kRawAmbientLightType = 33171111;
...
bool IsRawAmbientLightSensor(const android::hardware::sensors::V2_1::SensorInfo& sensor) {
    return static_cast<int32_t>(sensor.type) == kRawAmbientLightType;
}
...
disp_event_resp* parseDispEvent(int fd) {
    ...
    if (header.length < sizeof(disp_event_resp) || header.length > 4096) {
        LOG(ERROR) << "Invalid display event length: " << header.length;
        return nullptr;
    }
...
Return<Result> SensorsSubHal::activate(int32_t sensor_handle, bool enabled) {
    ...
    if (it != handle_type_.end() && static_cast<int32_t>(it->second) == kRawAmbientLightType) {
        is_raw_light = true;
...
void SensorsSubHal::displayMonitorThread() {
    ...
    while (!stop_disp_thread_.load()) {
        int rc = poll(&dispEventPoll, 1, 500);
        if (rc < 0) {
            if (errno == EINTR) {
                continue;
            }
        ...
        if (rc == 0) continue;
        if (dispEventPoll.revents & (POLLERR | POLLHUP | POLLNVAL)) break;
        if (!(dispEventPoll.revents & POLLIN)) continue;
...
void SensorsSubHal::postEvents(const std::vector<Event>& events, ScopedWakelock wakelock) {
    ...
    for (auto&& e : events) {
        ...
        if (static_cast<int32_t>(e.sensorType) == kRawAmbientLightType) {
            ...
            if (alias_handle != e.sensorHandle) {
                ...
                // SSC ambient_light_raw reports lux first, followed by raw ADC.
                event_copy.u.scalar = e.u.vec4.x;
